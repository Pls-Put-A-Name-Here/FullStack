...
def initiate_payment(request, order_id):
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({'error': 'Error initializing payment'}, status=response.status_code)
    ...
    if response.status_code == 200:
        ...
        if response_data['data']['status'] == 'success':
            ...
            if response_data['data']['amount'] == int(order.total_cost * 100):
                ...
                return JsonResponse({'status': 'failed', 'error': 'Amount mismatch'}, status=400)
        else:
            return JsonResponse({'status': 'failed', 'error': 'Payment failed'}, status=400)
    else:
        return JsonResponse({'status': 'failed', 'error': 'Verification failed'}, status=500)
